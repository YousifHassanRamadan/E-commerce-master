...
import {
  editProduct,
  resetEditState,
} from "../../../reduxSystem/slices/editProductSlice";

const EditProduct = ({ item, setIsOpen }) => {
  const dispatch = useDispatch();

  const { productEdited } = useSelector((state) => state.editProductState);

  const { Allcategories } =
    useSelector((state) => state.getCategoryState) || [];

  const { title, price, description, stock, category, thumbnail, images, _id } =
    item;
  ...
  useEffect(() => {
    if (productEdited) {
      ...
      dispatch(resetEditState()); // إعادة تعيين الحالة بعد الإشعار
      setIsOpen(false);
    }
  }, [productEdited]);
  ...
  const handleSubmit = () => {
    ...
    // console.log("Product Data after Edit:", product);
    ...
    <div className="flex justify-center items-center w-full min-h-screen bg-transparent p-4 z-[10000]">
      <div className="relative w-full sm:max-w-lg lg:max-w-5xl xl:max-w-6xl bg-white shadow-xl p-6 rounded-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {/* Thumbnail Upload */}
          <div className="border p-3 rounded-lg flex items-center justify-center relative cursor-pointer">
            <input
              type="file"
